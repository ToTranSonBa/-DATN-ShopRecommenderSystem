...
const HeaderNotificationBar = ({ className_bg, content, className_textcolor }) => {
    return (
        <div className={`items-center justify-center gap-2 px-1 py-1 flex lg:py-1 ${className_bg}`}>
            ...
            <span className={`text-sm font-normal md:text-md ${className_textcolor}`}>{content}— Limited time! </span>
            <a href="#" className={`flex items-center text-sm font-normal md:text-md text-primary lg:underline`}>
                Shop now
                <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
