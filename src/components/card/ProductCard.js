...
const ProductCard = ({ product }) => {
    return (
        <div key={product.iD_NK} className="bg-white shadow-sm hover:shadow-lg rounded-lg max-w-sm hover:border hover:border-blue-700">
            <a href={`/productdetail/${product.iD_NK}`}>
                ...
                <img className="rounded-t-lg p-2 sm:p-4 h-72" src={product.image} alt="product image"/>
            </a>
            <div className="p-3 sm:p-5 align-bot items-baseline">
                <a href={`/productdetail/${product.iD_NK}`}>
                    <h3 className="text-gray-900 font-semibold text-sm sm:text-lg limit-text">{product.name}</h3>
                </a>
