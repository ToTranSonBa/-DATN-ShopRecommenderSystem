...
import { CBFStatus, trainCBF, CancelCBF } from '../../../services/AdminApi/adminApi';

const AlgorithmContentBase = () => {
    ...
    const handle_cancel = async () => {
        const cancel_request = await CancelCBF();
    };

    const handle_start = async () => {
        const start_request = await trainCBF();
        setSeconds(0);
        setHours(0);
        setMinutes(0);
        setIsTraining(true);
    ...
    const handle_status = async () => {
        const status_request = await CBFStatus();
...
export default AlgorithmContentBase;
