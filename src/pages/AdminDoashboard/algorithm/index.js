...
import {trainNBCF, trainCancel, trainStatus} from '../../../services/AdminApi/adminApi'
...
const Algorithm = () => {
    ...
    const handle_cancel = async () => {
        const cancel_request = await trainCancel()
        setIsTraining(false)
    }
    ...
    const handle_status = async () => {
        const status_request = await trainStatus()
        setProgress(0);
            ...
            <div class="p-4 border-2 h-full border-dashed rounded-lg dark:border-gray-700 mt-20">
                ...
                <div class="grid grid-cols-3 gap-4 mb-4">
                    ...
                    <div class="flex justify-stretch items-center lg:gap-4  rounded  h-28 dark:bg-gray-800">
                        {btnstart && <button className="w-full text-white rounded-md cursor-pointer bg-primary lg:px-4 lg:py-2" onClick={handle_start}>
                            Cập nhật
                        </button>}
                        {btncancel && <button className="w-full text-white bg-red-600 rounded-md lg:px-4 lg:py-2" onClick={handle_cancel}>Huỷ</button>}
                    </div>
                </div>
            </div>
        </div>
