...
const ProductDetailPage = () => {
  ...
  const fetchCommentRating = useCallback(async () => {
    try {
      const response = await axios.get(
        `https://localhost:7016/api/DetailComments/RattingCount/Product${id}`
      );
  ...
  const fetchComments = useCallback(async () => {
    try {
      const response = await axios.get(
        `https://localhost:7016/api/DetailComments/Product${id}`
      );
    ...
    <div class="lg:pt-36 m-auto bg-background flex flex-col justify-center items-center pt-20">
              ...
              <div className="grid grid-cols-5 gap-6 mt-4">
                {productDetail?.productChildren?.map((option, index) => (
                  <button
                    ...
                    onClick={() => (
                      setSelectedOptionIndex(index), setPrice(option.price)
                    )}
                  >
      ...
      <div class="w-3/4 p-6 mt-8 grid items-start grid-cols-1 lg:grid-cols-7 gap-8 bg-white">
        <div class="lg:col-span-3 ">
          <div class="flex items-start w-full gap-4">
            <img
              src={
                productDetail.seller?.imageUrl || Default_Avatar
              }
      ...
      <div class="w-3/4 p-6 mt-8 bg-white">
        ...
        <div className="flex items-center justify-center">
          <button
            className="mt-6 p-1 justify-center underline underline-offset-2 hover:text-blue-500"
      ...
      <div class="w-3/4 p-6 my-8 bg-white">
        ...
        {comments.detailComment?.length !== 0 && (
          <CommentDetail comments={comments.detailComment} />
        )}
      </div>
    </div>
  );
